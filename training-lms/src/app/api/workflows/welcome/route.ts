...
export async function POST(request: NextRequest) {
  try {
    ...
    const { traineeEmail, traineeName } = body
    ...
    // Get n8n webhook URL
    const n8nWebhookUrl = process.env.N8N_WEBHOOK_WELCOME ||
      `${process.env.N8N_WEBHOOK_BASE_URL}/trigger/welcome`
    ...
    return NextResponse.json({
      success: true,
      message: `Welcome email workflow triggered for ${traineeEmail}`,
      result,
    })
  } catch (error) {
