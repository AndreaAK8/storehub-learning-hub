...
export async function POST(request: NextRequest) {
  try {
    ...
    const { traineeEmail, traineeName, feedbackType } = body
    ...
    // Get n8n webhook URL
    const n8nWebhookUrl = process.env.N8N_WEBHOOK_FEEDBACK ||
      `${process.env.N8N_WEBHOOK_BASE_URL}/trigger/feedback`
    ...
    return NextResponse.json({
      success: true,
      message: `Feedback workflow triggered for ${traineeEmail}`,
      result,
    })
  } catch (error) {
