...
export function VideoPlayer({ url, title, className = '' }: VideoPlayerProps) {
  ...
  if (!embedUrl) {
    return (
      <div className={`bg-[#eae9e8] rounded-lg p-8 text-center ${className}`}>
        <div className="text-[#a09d9a] mb-2">
  ...
  if (isForm) {
    return (
      <div className={`relative ${className}`}>
        {/* Platform badge */}
        {platform && (
          <div className="absolute top-2 right-2 z-10 bg-[#55504a] text-white text-xs px-2 py-1 rounded flex items-center gap-1">
        ...
        <iframe
          ...
          className="w-full rounded-lg border border-[#c5c3c1]"
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <iframe
        src={embedUrl}
        title={title || 'Video'}
        className="w-full aspect-video rounded-lg border border-[#c5c3c1]"
